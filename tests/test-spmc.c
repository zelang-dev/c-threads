...
spmc_ref_t spmc_new(size_t initial_cap, spmc_destructor_t destructor) {
	...
	const uint8_t power = initial_cap ? (uint8_t)initial_cap : DEFAULT_INITIAL_POWER;
	...
	for (i = 0; i < N_MC_ITEMS; i++)
		atomic_init(&observed_count[i], 0);
	return spmc;
}
...
void spmc_delete(spmc_ref_t spmc) {
	...
	size_t i;
	if (spmc->destructor) {
		for (node = atomic_get(spmc_node_t *, &head->next); node != head;
			prev = node, node = atomic_get(spmc_node_t *, &node->next), free(prev))
			for (i = atomic_get(size_t, &node->front); IS_READABLE(i, node); ++i)
...
static int mc_thread(void *arg) {
	...
	for (;;) {
		...
		} else if (atomic_fetch_add(&observed_count[element], 1)) {
...
#define N_MC_THREADS 5
int main() {
	spmc_ref_t spmc = spmc_new(0, NULL);
	thrd_t mc[N_MC_THREADS], producer;
	int i, r = 0;
	thrd_create(&producer, producer_thread, spmc);
	for (i = 0; i < N_MC_THREADS; i++) {
		thrd_create(&mc[i], mc_thread, spmc);
	}

	i = 0;
	thrd_join(producer, &i);
	for (i = 0; i < N_MC_THREADS; i++)
		thrd_join(mc[i], &r);

	for (i = 0; i < N_MC_ITEMS; ++i) {
		CHK_EXPECTED(atomic_load(&observed_count[i]), 1);
		continue;
	}
